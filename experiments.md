...
| 2026-09-10 | Image baseline v6 (multi-plane) | Kaggle (CPU) | Same as v5, but 2 slices from each of Sagittal/Axial/Coronal (6 total) instead of 5 slices from one plane | 0.547 | `<reverted>` | **No improvement over v5's 0.548** — essentially identical, despite ~40% longer runtime (18 vs 13 min). Reverted `kaggle/image_baseline.ipynb` back to the v5 single-plane multi-slice version (simpler, faster, same score) — this experiment's code is preserved in commit `19c2959` if revisited. Ensemble with text barely moved either (0.608, best weight now ~0.5-0.6). |
